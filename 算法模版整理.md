...
- 如果`i % primes[j] != 0`，`primes[j]`一定小于`i`的所有质因子，`primes[j]`同时也是`primes[j] * i`的最小质因子。综合以上两点，可以得出，在第12行**筛掉某个数时，一定是用其最小质因子筛去**
- 任何一个合数`x`，它一定会被筛掉。根据算术基本定理，一个大于1的正整数一定存在最小质因子，假设最小质因子为`primes[j]`，当`i`枚举到`x / primes[j]`时，`x`会在第12行被筛掉
